...
async function enviarMensajeBusiness(provider, destinatario, mensaje) {
  try {
    ...
    console.log(`📤 ENVIANDO A BUSINESS: ${destinatarioNormalizado}`);
    
    // 🔧 PAUSA PARA ESTABILIDAD
    await new Promise(resolve => setTimeout(resolve, 500));
    
    await sock.sendMessage(destinatarioNormalizado, { 
      text: mensaje,
      
      // 🔧 METADATOS PARA BUSINESS
      contextInfo: {
        isForwarded: false,
        forwardingScore: 0
      }
    ...
    console.log('✅ Mensaje enviado correctamente a Business');
    return true;
    
  } catch (error) {
    console.error('❌ Error enviando a Business:', error.message);
...
async function actualizarEstado(ctx, state, nuevoEstado, metadata = {}) { // 🔧 AGREGAR ctx COMO PARÁMETRO
  try {
    const estadoActual = await state.getMyState();
    const userPhone = ctx.from; // 🔧 USAR ctx.from EN LUGAR DE state.id

    // 🔧 VALIDAR QUE userPhone NO SEA NULL
    if (!userPhone) {
      console.error('❌ ctx.from es null en actualizarEstado');
      return;
    }
    ...
    // Solo guardar propiedades simples en metadata
    Object.keys(metadata).forEach(key => {
      ...
      if (valor === null || 
          ...
          (Array.isArray(valor)) ||
          (typeof valor === 'object' && 
           !(valor instanceof Function) && 
           !(valor instanceof Promise) &&
           !(valor?.constructor?.name === 'Timeout') &&
           !(valor?.constructor?.name === 'Socket')
          )) {
        ...
        try {
          ...
          console.log(`⚠️ Excluyendo propiedad circular/compleja: ${key}`);
          metadataLimpio[key] = `[${typeof valor}]`;
        }
      } else {
        console.log(`⚠️ Excluyendo propiedad compleja: ${key} (${typeof valor})`);
      }
    });
...
// ==== Función para actualizar contraseña en usuariosprueba - CORREGIDA ====
async function actualizarContrasenaEnusuariosprueba(usuario, nuevaContrasena, telefono) {
  try {
    ...
    if (result.affectedRows > 0) {
      console.log(`✅ Contraseña actualizada en usuariosprueba para usuario: ${usuario}`);
      return true;
    } else {
...
// ==== Función para normalizar IDs de WhatsApp Business ====
function normalizarIdWhatsAppBusiness(id) {
  if (!id) return id;

  // Si es un ID de grupo, dejarlo como está
  if (id.includes('@g.us')) {
    return id;
  }
  ...
  // Si es un número sin formato, agregar el sufijo correcto
  if (/^\d+$/.test(id)) {
    return `${id}@s.whatsapp.net`;
  }

  // Si ya tiene formato pero incorrecto, corregirlo
  if (id.includes('@c.us')) {
...
// ==== FLUJO INTERCEPTOR GLOBAL - CORREGIDO PARA WHATSAPP BUSINESS ====
const flowInterceptorGlobal = addKeyword(EVENTS.WELCOME)
  .addAction(async (ctx, { state, flowDynamic, gotoFlow, provider }) => {
    ...
    console.log(`🔍 INTERCEPTOR BUSINESS - Usuario: ${remitenteNormalizado}`);
    ...
    // 🔧 SOLO BLOQUEAR SI NO ES UN COMANDO VÁLIDO
    console.log(`🚫 Mensaje bloqueado: "${input}" - Estado: ${myState?.estadoUsuario}`);
    
    // 🔧 USAR FUNCIÓN MEJORADA PARA BUSINESS
    await enviarMensajeBusiness(provider, ctx.from, [
      '🔒 *Bot Inactivo*',
      '',
...
const main = async () => {
  try {
    ...
    const adapterProvider = createProvider(BaileysProvider, {
      ...
      // 🔧 CONFIGURACIÓN BROWSER OPTIMIZADA CON CHROME
      browser: ['Chrome', 'Windows', '10.0'], // ✅ CONFIGURACIÓN ESTABLE
      ...
      logger: {
        level: 'debug' // 🔧 CAMBIADO: 'debug' para diagnóstico, luego a 'error'
      },

      // 🔧 CONFIGURACIONES DE RECONEXIÓN (MEJORADAS)
      reconnect: true,
      maxRetries: 5,
      connectTimeoutMs: 60000, // 🔧 AUMENTADO: 60 segundos
      keepAliveIntervalMs: 25000, // 🔧 AUMENTADO: 25 segundos
      ...
      emitOwnEvents: false,
      defaultQueryTimeoutMs: 45000, // 🔧 AUMENTADO: 45 segundos
      ...
      // 🔧 ELIMINADO: configuraciones duplicadas y mediaCache extra
    });
