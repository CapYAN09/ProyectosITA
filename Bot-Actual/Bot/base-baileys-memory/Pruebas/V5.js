...
async function crearConexionRemota() {
  try {
    const connection = await mysql.createConnection({
      host: '172.30.247.185',
      user: 'ccomputo', // 🔧 Cambia si es necesario
      password: 'Jarjar0904$', // 🔧 Cambia si es necesario
    ...
    console.log('✅ Conexión a BD remota creada exitosamente');
    return connection;
  } catch (error) {
    console.error('❌ Error creando conexión a BD remota:', error.message);
    return null;
  }
}
...
// ==== FUNCIÓN PARA INSERTAR USUARIO EN TABLA REMOTA ====
async function insertarUsuarioEnBD(nombreCompleto, area, usuario, contrasena, telefono) {
  try {
    ...
    try {
      const [tablas] = await conexionRemota.execute(`
        SELECT TABLE_NAME 
        FROM INFORMATION_SCHEMA.TABLES 
        WHERE TABLE_SCHEMA = 'b1o04dzhm1guhvmjcrwb' 
        AND TABLE_NAME = 't_usuarios_sistema'
      `);

      if (tablas.length === 0) {
        console.log('📦 Creando tabla t_usuarios_sistema en BD remota...');
        await conexionRemota.execute(`
          CREATE TABLE t_usuarios_sistema (
        ...
        `);
        console.log('✅ Tabla t_usuarios_sistema creada exitosamente');
      }
    } catch (error) {
      console.error('❌ Error verificando/creando tabla:', error.message);
      return false;
    }

    // Insertar el usuario
    const query = `
      INSERT INTO t_usuarios_sistema (nombre_completo, area, usuario, contrasena, telefono)
      VALUES (?, ?, ?, ?, ?)
    `;

    const values = [nombreCompleto, area, usuario, contrasena, telefono];

    const [result] = await conexionRemota.execute(query, values);

    console.log(`✅ Usuario insertado en BD remota: ${usuario} - ID: ${result.insertId}`);
    return true;

  } catch (error) {
    console.error('❌ Error insertando usuario en BD remota:', error.message);

    // Si es error de duplicado, intentar con un usuario único
    if (error.code === 'ER_DUP_ENTRY') {
      console.log('🔄 Usuario duplicado, generando variante...');
      const usuarioUnico = `${usuario}_${Date.now().toString().slice(-4)}`;
      return await insertarUsuarioEnBD(nombreCompleto, area, usuarioUnico, contrasena, telefono);
    }
...
// ==== CONEXIÓN A BASE DE DATOS REMOTA PARA t_usuarios ====
let conexionRemotaTUsuarios = null;

async function crearConexionRemotaTUsuarios() {
  try {
    const connection = await mysql.createConnection({
      host: '172.30.247.185',
      user: 'ccomputo',
      password: 'Jarjar0904$',
      database: 'b1o04dzhm1guhvmjcrwb', // 🔧 Usando la misma BD que ya tienes configurada
      port: 3306,
      connectTimeout: 60000,
      acquireTimeout: 60000,
      timeout: 60000,
    ...
    console.log('✅ Conexión a BD remota t_usuarios creada exitosamente');
    return connection;
  } catch (error) {
    console.error('❌ Error creando conexión a BD remota t_usuarios:', error.message);
    return null;
  }
}

async function inicializarConexionRemotaTUsuarios() {
  if (!conexionRemotaTUsuarios) {
    conexionRemotaTUsuarios = await crearConexionRemotaTUsuarios();
  }

  // Verificar si la conexión sigue activa
  if (conexionRemotaTUsuarios) {
    try {
      await conexionRemotaTUsuarios.execute('SELECT 1');
    } catch (error) {
      console.log('🔄 Conexión remota t_usuarios inactiva, reconectando...');
      conexionRemotaTUsuarios = await crearConexionRemotaTUsuarios();
    }
  }

  return conexionRemotaTUsuarios;
}

// ==== FUNCIÓN PARA VERIFICAR TABLA t_usuarios ====
async function verificarTablaTUsuarios() {
  try {
    await inicializarConexionRemotaTUsuarios();

    if (!conexionRemotaTUsuarios) {
      console.error('❌ No hay conexión a BD remota para verificar tabla');
      return false;
    }

    const [tablas] = await conexionRemotaTUsuarios.execute(`
      SELECT TABLE_NAME 
      FROM INFORMATION_SCHEMA.TABLES 
      WHERE TABLE_SCHEMA = 'b1o04dzhm1guhvmjcrwb' 
    ...
    if (tablas.length > 0) {
      console.log('✅ Tabla t_usuarios encontrada en la BD remota');
      return true;
    } else {
      console.log('❌ Tabla t_usuarios NO encontrada en la BD remota');
      return false;
    }

  } catch (error) {
    console.error('❌ Error verificando tabla t_usuarios:', error.message);
    return false;
  }
}

// ==== FUNCIONES PARA GESTIONAR t_usuarios ====
async function consultarUsuarioEnTUsuarios(numeroControl) {
  try {
    await inicializarConexionRemotaTUsuarios();

    if (!conexionRemotaTUsuarios) {
      console.error('❌ No hay conexión a BD remota t_usuarios');
      return null;
    }

    // Primero verificamos si la tabla existe
    const tablaExiste = await verificarTablaTUsuarios();
    if (!tablaExiste) {
      console.error('❌ La tabla t_usuarios no existe en la BD');
      return null;
    }

    const query = `
      SELECT id_usuario, id_rol, id_persona, usuario, password, ubicacion, fecha_insert, Estado 
      FROM t_usuarios 
      WHERE usuario LIKE ? OR usuario = ? OR id_usuario = ?
    `;

    const [rows] = await conexionRemotaTUsuarios.execute(query, [
      `%${numeroControl}%`,
      numeroControl,
      numeroControl
    ]);
    ...
    if (rows.length > 0) {
      console.log(`✅ Usuario encontrado en t_usuarios: ${rows[0].usuario}`);
      return rows[0];
    } else {
      console.log('❌ Usuario no encontrado en t_usuarios');
      return null;
    }

  } catch (error) {
    console.error('❌ Error consultando usuario en t_usuarios:', error.message);
    return null;
  }
}

async function actualizarPasswordEnTUsuarios(idUsuario, nuevaPassword) {
  try {
    await inicializarConexionRemotaTUsuarios();
    ...
    const query = `
      UPDATE t_usuarios 
      SET password = ?, fecha_insert = ?
      WHERE id_usuario = ?
    `;

    const fechaActual = new Date().toISOString().slice(0, 19).replace('T', ' ');

    const [result] = await conexionRemotaTUsuarios.execute(query, [nuevaPassword, fechaActual, idUsuario]);

    if (result.affectedRows > 0) {
      console.log(`✅ Contraseña actualizada en t_usuarios para ID: ${idUsuario}`);
      return true;
    } else {
      console.log(`❌ No se pudo actualizar contraseña en t_usuarios para ID: ${idUsuario}`);
      return false;
    }

  } catch (error) {
    console.error('❌ Error actualizando contraseña en t_usuarios:', error.message);
    return false;
  }
}
...
async function listarTodosLosUsuarios() {
  try {
    ...
    const query = `
      SELECT id_usuario, usuario, id_rol, ubicacion, fecha_insert, Estado 
      FROM t_usuarios 
      ORDER BY id_usuario 
      LIMIT 50
    `;
    ...
    console.log(`✅ Se encontraron ${rows.length} usuarios en t_usuarios`);
    return rows;

  } catch (error) {
    console.error('❌ Error listando usuarios en t_usuarios:', error.message);
    return null;
  }
}

// ==== FLUJO MEJORADO PARA OPCIÓN 7 - GESTIÓN DE SERVICIOS ====
/*
const flowGestionServicios = addKeyword(EVENTS.ACTION)
  .addAnswer(
    [
      ...
      '3️⃣ 🔍 Consultar información de usuario en sistema',
      '',
    ...
    async (ctx, { flowDynamic, gotoFlow, state }) => {
      ...
      if (opcion === '3') {
        await flowDynamic('🔍 Iniciando consulta de información de usuario...');
...
// ==== NUEVO FLUJO PARA CONSULTA DE USUARIOS EN t_usuarios ====
// ==== FLUJO MEJORADO PARA CONSULTA DE USUARIOS EN t_usuarios ====
const flowConsultaUsuario = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { state, flowDynamic, gotoFlow }) => {
    ...
    const timeout = timeoutManager.setTimeout(userPhone, async () => {
      try {
        console.log('⏱️ Timeout de 2 minutos en consulta usuario');
  ...
  .addAnswer(
    '🔍 *CONSULTA DE USUARIOS - BASE DE DATOS REMOTA* 🔍\n\n' +
    'Selecciona una opción:\n\n' +
    '1️⃣ 🔎 Buscar usuario específico\n' +
    '2️⃣ 📋 Listar todos los usuarios (máx. 50)\n\n' +
...
// ==== FLUJO PARA BUSCAR USUARIO ESPECÍFICO ====
const flowBuscarUsuarioEspecifico = addKeyword(EVENTS.ACTION)
  .addAnswer(
    '🔎 Escribe el *ID de usuario, número de control o nombre de usuario* a buscar:',
    { capture: true },
    async (ctx, { flowDynamic, gotoFlow, state }) => {
      if (ctx.from === CONTACTO_ADMIN) return;
      ...
      // Realizar consulta en la base de datos remota
      await flowDynamic('🔍 Consultando información en la base de datos remota (172.30.247.185)...');

      const usuario = await consultarUsuarioEnTUsuarios(input);
      ...
      if (usuario) {
        await flowDynamic([
          ...
          `📅 Fecha de inserción: ${usuario.fecha_insert}`,
        ...
        await flowDynamic([
          ...
          '',
          'El usuario no fue encontrado en la base de datos remota.',
          '',
          '💡 **Verifica:**',
          '• El ID de usuario',
...
// ==== FLUJO PARA LISTAR TODOS LOS USUARIOS ====
const flowListarTodosUsuarios = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { flowDynamic, gotoFlow, state }) => {
    if (ctx.from === CONTACTO_ADMIN) return;

    await flowDynamic('📋 Obteniendo lista de usuarios de la base de datos remota (172.30.247.185)...');

    const usuarios = await listarTodosLosUsuarios();

    if (usuarios && usuarios.length > 0) {
      let mensaje = '📊 *LISTA DE USUARIOS - BASE DE DATOS REMOTA*\n\n';

      usuarios.forEach((usuario, index) => {
        mensaje += `*${index + 1}.* 🆔 ${usuario.id_usuario} | 👤 ${usuario.usuario} | 🎯 Rol: ${usuario.id_rol} | 🔰 ${usuario.Estado === 1 ? 'Activo' : 'Inactivo'}\n`;

        // Dividir en múltiples mensajes si es muy largo
        if ((index + 1) % 10 === 0) {
          flowDynamic(mensaje);
          mensaje = ''; // Reiniciar para el siguiente mensaje
        }
      });
      ...
      await flowDynamic([
        '',
        `✅ Total de usuarios mostrados: ${usuarios.length}`,
        '🔗 *Base de datos: 172.30.247.185*',
        '',
...
const flowInterceptorGlobal = addKeyword(EVENTS.WELCOME)
  .addAction(async (ctx, { state, flowDynamic, gotoFlow, endFlow }) => {
    ...
    // 🔧 CORRECCIÓN: Si el usuario está en menú o libre, permitir cualquier mensaje
    if (!myState?.estadoUsuario ||
      myState.estadoUsuario === ESTADOS_USUARIO.LIBRE ||
      myState.estadoUsuario === ESTADOS_USUARIO.EN_MENU) {
      return endFlow(); // 🔧 PERMITIR que el mensaje pase al flujo principal
    }
    ...
    // 🔧 SOLO mostrar mensaje de inactivo si el estado no es reconocido
    await flowDynamic([
      '🔒 *Bot Inactivo*',
      '',
...
// ==== FLUJO PARA CAPTURAR ÁREA (ACTUALIZADO CON INSERCIÓN EN BD) ====
const flowCapturaArea = addKeyword(EVENTS.ACTION)
  ...
  .addAnswer(
    ...
    async (ctx, { flowDynamic, gotoFlow, state, provider }) => {
      ...
      // ✅ INSERTAR EN BASE DE DATOS REMOTA
      const insercionExitosa = await insertarUsuarioEnBD(
        nombreCompleto,
        area,
        nuevoUsuario,
        nuevaContrasena,
      ...
      if (envioExitoso) {
        await flowDynamic([
          ...
          `👤 Nombre: ${nombreCompleto}`,
          `🏢 Área: ${area}`,
          `💾 *Estado BD:* ${insercionExitosa ? '✅ Registrado' : '⚠️ Pendiente'}`,
          '',
      ...
      const intervalId = setInterval(async () => {
        ...
        if (minutosRestantes > 0) {
          try {
            await flowDynamic(`⏳ Hola *${nombreCompleto}*, faltan *${minutosRestantes} minutos* para completar la creación de tu usuario...`);
          } catch (error) {
      ...
      const timeoutId = setTimeout(async () => {
        ...
        try {
          ...
          await flowDynamic([
            ...
            `💾 *Base de datos:* ${insercionExitosa ? '✅ Registrado' : '⚠️ Contactar soporte'}`,
            '',
...
// ==== FUNCIÓN PARA ACTUALIZAR CONTRASEÑA EN BD REMOTA ====
async function actualizarContrasenaEnBD(usuario, nuevaContrasena, telefono) {
  try {
    await inicializarConexionRemota();
    ...
    const query = `
      UPDATE t_usuarios_sistema 
      SET contrasena = ?, fecha_actualizacion = CURRENT_TIMESTAMP 
      WHERE usuario = ? OR telefono = ?
    `;

    const values = [nuevaContrasena, usuario, telefono];

    const [result] = await conexionRemota.execute(query, values);

    if (result.affectedRows > 0) {
      console.log(`✅ Contraseña actualizada en BD remota para usuario: ${usuario}`);
      return true;
    } else {
      console.log(`⚠️ Usuario no encontrado en BD remota: ${usuario}`);
      return false;
    }

  } catch (error) {
    console.error('❌ Error actualizando contraseña en BD remota:', error.message);
    return false;
  }
}

// ==== FLUJO PARA CAPTURAR USUARIO DEL SISTEMA (ACTUALIZADO CON BD) ====
const flowCapturaUsuarioSistema = addKeyword(EVENTS.ACTION)
  ...
  .addAnswer(
    ...
    async (ctx, { flowDynamic, gotoFlow, state, provider }) => {
      ...
      // ✅ ACTUALIZAR CONTRASEÑA EN BASE DE DATOS REMOTA
      const actualizacionExitosa = await actualizarContrasenaEnBD(
        usuarioSistema,
        nuevaContrasena,
        ctx.from
      );
...
const main = async () => {
  try {
    ...
    const adapterProvider = createProvider(BaileysProvider, {
      ...
      // 🔧 CONFIGURACIÓN ESPECÍFICA PARA BUSINESS
      browser: ['ITA-Center-Bot', 'Chrome', '1.0.0'], // Nombre profesional
      ...
      logger: {
        level: 'error' // Niveles válidos: 'fatal', 'error', 'warn', 'info', 'debug', 'trace'
      },
      ...
      // 🔧 CONFIGURACIÓN DE MEDIOS PARA BUSINESS
      mediaCache: {
        maxItems: 30,
