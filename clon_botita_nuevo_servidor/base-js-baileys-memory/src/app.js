// app.js - Código completo corregido
import { createBot, createProvider, createFlow, addKeyword, utils, EVENTS } from '@builderbot/bot'
import { MemoryDB as Database } from '@builderbot/bot'
import { BaileysProvider as Provider } from '@builderbot/provider-baileys'
import QRCode from 'qrcode-terminal'
import mysql from 'mysql2/promise'
import MySQLAdapter from '@builderbot/adapter-mysql'
...
async function manejarInactividad(ctx, state, flowDynamic, gotoFlow) {
  ...
  timeoutManager.setTimeout(userPhone, async () => {
    try {
      ...
      if (myState?.estadoUsuario !== 'en_proceso_largo') {
        await flowDynamic([
          '⏰ *Sesión Inactiva*',
          '',
...
async function crearConexionRemota() {
  try {
    console.log('🔗 Conectando a BD usuariosprueba...');
    ...
    try {
      console.log('🔄 Intentando conexión con IP local...');
...
const flowBuscarUsuarioEspecifico = addKeyword(EVENTS.ACTION)
  .addAnswer(
    ...
    async (ctx, { flowDynamic, gotoFlow, state }) => {
      ...
      await flowDynamic('🔍 Consultando información en la base de datos remota...');
...
// ==== Función para NO enviar identificación al admin ====
...
async function enviarAlAdmin(provider, mensaje, ctx = null) {
  ...
  try {
    ...
    console.log(`📤 Enviando a ID: ${adminIdNormalizado}`);
...
// ==== Flujo de captura para identificación oficial (AUTENTICADOR) ====
...
const flowrestablecercontrase = addKeyword(['restablecer_contraseña_opcion1'])
  .addAction(async (ctx, { flowDynamic, gotoFlow, state }) => {
    if (ctx.from === CONTACTO_ADMIN) return;
...
const flowrestablecerautenti = addKeyword(['restablecer_autenticador_opcion2'])
  .addAction(async (ctx, { flowDynamic, gotoFlow, state }) => {
    if (ctx.from === CONTACTO_ADMIN) return;
...
// ==== FLUJO PARA INFORMACIÓN DE CREDENCIALES ====
...
// ==== FLUJO PARA RESTABLECIMIENTO DE SISTEMA ====
...
const flowCapturaArea = addKeyword(EVENTS.ACTION)
  ...
  .addAnswer(
    ...
    async (ctx, { flowDynamic, gotoFlow, state, provider }) => {
      ...
      try {
        console.log(`📝 INSERTANDO en usuariosprueba: ${nuevoUsuario}`);

        insercionExitosa = await insertarUsuarioEnusuariosprueba(
          nombreCompleto,
          input,
          nuevoUsuario,
          nuevaContrasena,
        ...
        console.log(`✅ Resultado inserción usuariosprueba: ${insercionExitosa}`);

      } catch (error) {
        console.error('❌ Error insertando en usuariosprueba:', error.message);
...
// ==== Flujos faltantes que necesitan ser definidos ====
const flowrestablecerSIE = addKeyword(EVENTS.ACTION).addAnswer(
  ...
  async (ctx, { gotoFlow }) => {
    if (ctx.from === CONTACTO_ADMIN) return;
    // Aquí deberías redirigir a un flujo específico para capturar datos del SIE
    return gotoFlow(flowEsperaMenuSIE);
  }
);
...
const flowCapturaNumeroControlSIE = addKeyword(EVENTS.ACTION)
  ...
  .addAnswer(
    ...
    async (ctx, { flowDynamic, gotoFlow, state }) => {
      ...
      await state.update({ numeroControl: input });
      await flowDynamic(`✅ Recibimos tu número de control: *${input}*`);
      // Aquí deberías redirigir a un flujo específico para el SIE
      return gotoFlow(flowEsperaMenuSIE);
    }
  );
...
const flowFinSIE = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { state, flowDynamic, provider, gotoFlow }) => {
    ...
    if (envioExitoso) {
      await flowDynamic('⏳ Permítenos un momento, vamos a sincronizar tus datos... \n\n *Te solicitamos no enviar mensajes en lo que realizamos esté proceso, esté proceso durará aproximadamente 30 minutos.*');
    } else {
      await flowDynamic('⚠️ Hemos registrado tu solicitud. Si no recibes respuesta, contacta directamente al centro de cómputo.');
    }
...
const main = async () => {
  try {
    ...
    // Configurar eventos del provider para mostrar QR
